...
public class HauntboundBootsLayer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {

    private final HauntboundBootsModel<T> model;

    public HauntboundBootsLayer(RenderLayerParent<T, M> renderLayerParent) {
        super(renderLayerParent);
        this.model = new HauntboundBootsModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(SleepyHollowsClient.HAUNTBOUND_BOOTS_LAYER));
    }
    ...
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource multiBufferSource, int i, @NotNull T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity instanceof Player player) {
            ItemStack bootsItem = player.getInventory().armor.get(0);
            boolean bootsSlotHasHauntboundBoots = bootsItem.getItem() instanceof HauntboundArmorItem;

            if (bootsSlotHasHauntboundBoots) {
    ...
    protected @NotNull ResourceLocation getTextureLocation(@NotNull T entity) {
        return HauntboundBootsModel.HAUNTBOUND_BOOTS_TEXTURE;
    }
}
